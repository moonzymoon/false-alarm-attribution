"""补全 RegimeGlobal 在 PCA/OCSVM 单元上的结果.

镜像 run_new_scorers.py 的完整单元构建流程(种子一致 → 单元逐位相同),
但只计算 RegimeGlobal 一个方法, 输出到 _cache/regimeglobal_new_scorers.json.
注意: 保留原脚本的 rng 消耗顺序, 不增删任何 rng 调用.
"""
...
# NSUnit 直接复用 run_new_scorers 的定义(import 会执行整个脚本, 故复制类定义)
class NSUnit:
    ...
    for ds in DATASETS:
        ...
        for scorer in SCORERS:
            ...
            for k in range(rm.K):
                ...
                print(f"    r{k}: n={len(ends)} → ", end="", flush=True)
                ...
                try:
                    ...
                    print(f"RegimeGlobal L1={m['layer1_macro_f1']:.3f} ({time.time()-t0:.0f}s)", flush=True)
                except Exception as e:
                    all_res[unit.name] = {"RegimeGlobal": {"error": f"{type(e).__name__}: {e}"}}
                    print(f"ERROR {type(e).__name__}: {e}", flush=True)

    out = cpath("regimeglobal_new_scorers.json")
    json.dump(all_res, open(out, "w", encoding="utf-8"), indent=1)
    print(f"\nsaved {len(all_res)} units → {out}  total {time.time()-t0:.0f}s", flush=True)

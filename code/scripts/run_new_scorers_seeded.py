"""C2 multi-seed variant: run_new_scorers with all seeds shifted by OFF."""
...
def run():
    ...
    with open(cpath("new_scorer_results_seed%d.json" % OFF), "w", encoding="utf-8") as f:
        json.dump(all_res, f, ensure_ascii=False, indent=1, default=float)
    print(f"{len(all_res)} units saved (seed offset {OFF})", flush=True)

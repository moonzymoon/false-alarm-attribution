"""SHAP variant: same deterministic unit construction (OFF=0), SHAP only."""
"""第4/5种检测器(PCA+OCSVM) × 4数据集 × 9归因方法 = 扩展匹配研究."""
import sys, os, json
import numpy as np

sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
import run_shap_attr as _rsa  # noqa
...
def run():
        ...
        for scorer in SCORERS:
            ...
            for k in range(rm.K):
                ...
                try:
                    ...
                    for mn in ("SHAP",):
                        try:
                            attr = _rsa.shap_attribute(unit, W, bank)
    ...
    with open(cpath("new_scorer_results_shap.json"), "w", encoding="utf-8") as f:
        json.dump(all_res, f, ensure_ascii=False, indent=1, default=float)
    print(f"{len(all_res)} units saved (SHAP)", flush=True)
